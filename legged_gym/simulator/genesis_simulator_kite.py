...
""" ********** KITE Genesis Simulator ********** """
class GenesisSimulator_KITE(Simulator):
    ...
    def _create_async_pino_workers(self):
        # KITE currently does not run the async Pinocchio dynamics path in
    ...
    def post_physics_step(self):
        ...
        if self._cfg.terrain.measure_heights:
            ...
            self._calc_surrounding_surface_normals()
    ...
    def draw_debug_vis(self):
        ...
        """
        if (
            self._cfg.terrain.measure_heights
            and getattr(
                self._cfg.terrain,
                "debug_draw_measured_surface_normals",
                False,
            )
        ):
    ...
    def draw_measured_surface_normals(self):
        """Draw measured terrain locations and their world-frame surface normals."""
        refresh_steps = max(
            1,
            int(
                getattr(
                    self._cfg.terrain,
                    "debug_surface_normal_refresh_steps",
                    5,
                )
            ),
        )
        last_update = getattr(
            self, "_surface_normal_debug_last_update", -refresh_steps
        )
        if self.common_step_counter - last_update < refresh_steps:
            return

        env_id = int(
            getattr(self._cfg.terrain, "debug_height_map_env_id", 0)
        )
        if not 0 <= env_id < self._num_envs:
            return

        # Reconstruct the world XY location of each robot-centric sample and
        # place the marker at the terrain height stored in _measured_heights.
        sample_points = quat_apply_yaw(
            self._base_quat[env_id : env_id + 1].repeat(
                1, self._num_height_points
            ),
            self._height_points[env_id : env_id + 1],
        )[0]
        ...
        new_debug_objects = []
        height_markers = self._scene.draw_debug_spheres(
            sample_points,
            radius=point_radius,
            color=tuple(
                getattr(
                    self._cfg.terrain,
                    "debug_height_point_color",
                    (0.0, 1.0, 0.0, 1.0),
                )
            ),
        )
        new_debug_objects.append(height_markers)

        normal_length = float(
            getattr(self._cfg.terrain, "debug_surface_normal_length", 0.12)
        )
        normal_radius = float(
            getattr(self._cfg.terrain, "debug_surface_normal_radius", 0.003)
        )
        normal_color = tuple(
            getattr(
                self._cfg.terrain,
                "debug_surface_normal_color",
                (1.0, 0.8, 0.0, 1.0),
            )
        )
        ...
        arrow_vectors = (
            self._measured_surface_normals[env_id] * normal_length
        )
        # Genesis computes arrow length with NumPy before converting inputs, so
        # arrows must cross the debug-rendering boundary as host arrays.
        arrow_origins = arrow_origins.detach().cpu().numpy()
        arrow_vectors = arrow_vectors.detach().cpu().numpy()
        for origin, vector in zip(arrow_origins, arrow_vectors):
            new_debug_objects.append(
        ...
        # Keep the previous overlay alive until the replacement is complete.
        # This avoids a blank rendered frame between clear and redraw.
        for debug_object in getattr(
            self, "_surface_normal_debug_objects", ()
        ):
    ...
    def _init_buffers(self):
        ...
        self._measured_surface_normals = torch.zeros(
            self._num_envs,
        ...
        self._measured_surface_normals[:, :, 2] = 1.0
    ...
    def _calc_surrounding_surface_normals(self):
        """Compute one world-frame terrain normal for each measured height point."""
        if self._cfg.terrain.mesh_type == 'plane':
            self._measured_surface_normals.zero_()
            self._measured_surface_normals[:, :, 2] = 1.0
            return
        if self._cfg.terrain.mesh_type == 'none':
            raise NameError(
                "Can't measure terrain normals with terrain mesh type 'none'"
            )

        # Rotate the robot-centric sample grid into the world frame, then use
        # neighboring global height-field cells for centered finite differences.
        ...
        px = points[:, :, 0].clamp(1, self._height_samples.shape[0] - 2)
        py = points[:, :, 1].clamp(1, self._height_samples.shape[1] - 2)

        height_x_low = self._height_samples[px - 1, py]
        height_x_high = self._height_samples[px + 1, py]
        height_y_low = self._height_samples[px, py - 1]
        height_y_high = self._height_samples[px, py + 1]

        height_scale = self._cfg.terrain.vertical_scale
        sample_span = 2.0 * self._cfg.terrain.horizontal_scale
        dz_dx = (height_x_high - height_x_low) * height_scale / sample_span
        dz_dy = (height_y_high - height_y_low) * height_scale / sample_span

        normals = torch.stack(
            (-dz_dx, -dz_dy, torch.ones_like(dz_dx)),
            dim=-1,
        ).float()
        self._measured_surface_normals[:] = F.normalize(
            normals, p=2, dim=-1
        )

    @property
    def measured_surface_normals(self):
