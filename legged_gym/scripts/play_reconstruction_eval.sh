...
Optional environment settings:
  CONDITION=payload           nominal | payload | impulse
  ...
  INJECTION_TIME=2 VELOCITY_DELTA="0 1 0 0 0 0"
...
ABL3_CHECKPOINT=${ABL3_CHECKPOINT:-logs/pact_corl/go1_abl3_rough/Sep11_00-04-14_hybrid_100hz_spec_materr/model_8000.pt}
...
read -r -a velocity_delta <<< "${VELOCITY_DELTA:-0 1 0 0 0 0}"
for source in pact abl3; do
  ...
  SIMULATOR="$backend" "${PYTHON[@]}" legged_gym/scripts/play_exp_decoder_eval.py collect \
    ...
    --injection-time "${INJECTION_TIME:-2}" --velocity-delta "${velocity_delta[@]}" \
