...
class Go2KITEBaseline(KITEDepthMixin, BaseTask):
    ...
    def compute_observations(self):
        ...
        self.critic_obs_buf_single[:, 0:57] = self.obs_buf
        self.critic_obs_buf_single[:, 57:60] = (
            self.simulator.base_lin_vel * self.obs_scales.lin_vel
        )
        self.critic_obs_buf_single[:, 60:72] = (
            self.simulator._grfs_buf * self.obs_scales.grf
        )
        self.critic_obs_buf_single[:, 72:84] = (
            self.simulator.normal_vector_around_feet.reshape(self.num_envs, -1)
        )
        self.critic_obs_buf_single[:, 84:88] = self.simulator.link_contact_states[
            :, self.simulator.feet_indices
        ]
        self.critic_obs_buf_single[:, 88:89] = self.simulator.feedforward_tau_weight
        self.critic_obs_buf_single[:, 89:90] = self.simulator.feedback_tau_weight
        self.critic_obs_buf_single[:, 90:141] = self.domain_randomization_info_buf
