...
#
###
class ContextEncoderRL2AC(nn.Module):
    ...
    def __init__(
        ...
        # RL2AC q_ref prediction layers
        qr_dim_h2 = int((output_hdim - 12)/2.0)
        self.ce_qr_h1 = nn.Linear(output_hdim, output_hdim)
        self.ce_qr_h2 = nn.Linear(output_hdim, qr_dim_h2)
        self.ce_qr_out = nn.Linear(qr_dim_h2, 12)
    ...
    def _initialize_weights(self) -> None:
        ...
        for layer in [self.ce_in, self.ce_h1,
                     ...
                     self.ce_qr_h1, self.ce_qr_h2, self.ce_qr_out]:
    ...
    def encode(self, X_C: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        qr_x = self.activation(self.ce_qr_h1(x))
        qr_x = self.activation(self.ce_qr_h2(qr_x))

        return self.ce_out_mean(lat_mean), self.ce_out_var(lat_var), self.ce_velo_mean(velo_mean), self.ce_velo_var(velo_var), self.ce_qr_out(qr_x)
...
class ActorCritic_RL2AC(nn.Module):
    def __init__(self, 
                 num_actor_obs=57, 
                 num_critic_obs=95, 
                 num_actions=12,
        ...
        self.context_encoder = ContextEncoderRL2AC(context_input_dim=cenet_in_dim,
    ...
    def cenet_enc_forward(self, obs_history):
        mean, logvar, z, torso_velo, _ = self.context_encoder(obs_history)
        return mean, logvar, z, torso_velo
    
    def cenet_enc_inference(self, obs_history):
        mean_z, mean_v, qr = self.context_encoder.forward_inference(obs_history)
        return mean_z, mean_v, qr
    ...
    def actor_forward(self, current_obs):
        ...
        actions = self.act_trunk(current_obs)
    ...
    def act_inference(self,obs,obs_history):
        # Call the forward method of the context encoder
        z, torso_velo, qr = self.cenet_enc_inference(obs_history)
